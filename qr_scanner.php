...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qr_data'])) {
    ...
    // Format: INV-SKU, or a raw SKU typed/scanned directly
    $sku = preg_match('/^INV-(.+)$/', $qr_data, $matches) ? $matches[1] : $qr_data;

    $stmt = $conn->prepare(
        "SELECT im.item_id, im.sku, im.item_name, im.category, im.unit, im.unit_cost,
                im.reorder_level, im.status,
                si.quantity_on_hand, si.quantity_reserved, si.warehouse_location
         FROM item_master im
         LEFT JOIN stock_inventory si ON im.item_id = si.item_id
         WHERE im.sku = ? LIMIT 1"
    );
    ...
    if ($result->num_rows > 0) {
        ...
        core_log(
            ...
            $_SESSION['username'],
            'QR Scan',
            "Scanned inventory item: {$scanned_item['item_name']} (SKU: {$scanned_item['sku']})"
        );
    } else {
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <main class="flex-1 overflow-y-auto bg-slate-50 p-6 md:p-8">
            <div class="max-w-4xl mx-auto space-y-8">
                ...
                <!-- Scanned Item Result -->
                <?php if ($scanned_item):
                    $onHand   = (int)($scanned_item['quantity_on_hand'] ?? 0);
                    $reserved = (int)($scanned_item['quantity_reserved'] ?? 0);
                ...
                ?>
                    <div class="result-card p-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div class="bg-slate-50 p-3 rounded-lg">
                                <p class="text-xs text-slate-500 uppercase tracking-wider">On Hand</p>
                                <p class="font-semibold text-slate-900"><?php echo $onHand; ?> <?php echo htmlspecialchars($scanned_item['unit']); ?></p>
                            </div>
                            <div class="bg-slate-50 p-3 rounded-lg">
                                <p class="text-xs text-slate-500 uppercase tracking-wider">Unit Cost</p>
                                <p class="font-semibold text-slate-900">₱<?php echo number_format((float)$scanned_item['unit_cost'], 2); ?></p>
                            </div>
                            <div class="bg-slate-50 p-3 rounded-lg">
                                <p class="text-xs text-slate-500 uppercase tracking-wider">Location</p>
                                <p class="font-semibold text-slate-900"><?php echo htmlspecialchars($scanned_item['warehouse_location'] ?: 'Unshelved'); ?></p>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-200">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <p class="text-xs text-slate-500 uppercase tracking-wider">Available (On Hand − Reserved)</p>
                                    <p class="font-medium <?php echo $available <= 0 ? 'text-rose-600' : 'text-slate-900'; ?>"><?php echo $available; ?> <?php echo htmlspecialchars($scanned_item['unit']); ?></p>
                                </div>
                                <div>
                        ...
                        <div class="mt-4 flex gap-2">
                            <a href="item_master.php" class="px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold">View in Item Master</a>
                            <a href="stock_levels.php?q=<?php echo urlencode($scanned_item['sku']); ?>" class="px-4 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-600">View Stock Levels</a>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
